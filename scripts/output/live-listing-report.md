# Live listing check — 2026-08-31

140 product listings checked. Drift threshold 10%.

| Status | Count | Meaning |
|---|---|---|
| **DEAD** | 0 | Amazon says the page is gone — the buy button goes nowhere |
| **NO_PRICE** | 3 | Listing exists but nothing is purchasable |
| **DRIFT** | 0 | Live price differs from the registry by >10% |
| OK | 9 | Priced and within tolerance |
| *could not check* | 128 | Blocked or errored — **not** a problem, just unknown |

## Needs attention

| Product | Status | Registry | Live | Drift | Note |
|---|---|---|---|---|---|
| `cobra-aerojet-max` | **NO_PRICE** | ~$399 | — | — | no price in the buy box (variant-select pages land here too — eyeball before acting) |
| `footjoy-weathersof-glove` | **NO_PRICE** | ~$22 | — | — | no price in the buy box (variant-select pages land here too — eyeball before acting) |
| `vice-pro` | **NO_PRICE** | ~$33/dozen | — | — | no price in the buy box (variant-select pages land here too — eyeball before acting) |
...
- `shot-scope-v5` — Amazon served a robot check — nothing learned, re-run later
...
- `eyeline-putting-cup` — Amazon served a robot check — nothing learned, re-run later
- `gustbuster-umbrella` — Amazon served a robot check — nothing learned, re-run later
- `odyssey-white-hot-og` — Amazon served a robot check — nothing learned, re-run later
...
- `fiberbuilt-studio-mat` — Amazon served a robot check — nothing learned, re-run later
...
- `callaway-big-bertha-b21-irons` — Amazon served a robot check — nothing learned, re-run later
...
- `taylormade-noodle` — Amazon served a robot check — nothing learned, re-run later
...
- `garmin-approach-s12` — Amazon served a robot check — nothing learned, re-run later
...
- `fiberbuilt-flight-deck` — Amazon served a robot check — nothing learned, re-run later
...
- `adidas-ultimate365-pants` — Amazon served a robot check — nothing learned, re-run later
...
- `callaway-rogue-st-max` — Amazon served a robot check — nothing learned, re-run later
